import ShiningStar from "./Assets/ShiningStar.jpeg";
import SaamaOneYear from "./Assets/One_Year_Saama.jpeg";
import Appreciation from "./Assets/Appreciation.jpg";
import GenerativeAI from "./Assets/Generative-AI.jpg";
import Hackathon from "./Assets/Hackathon.jpeg";
import Clothifyy from "./Assets/Clothify.jpg";
import Shopmart from "./Assets/ShopMart.png";
import Rental from "./Assets/Rental.png";
import GymHolic from "./Assets/GymHolic.png";
import LearnHub from "./Assets/LearnHub.jpeg";
import ExpenseMate from "./Assets/ExpenseMate.png";
import Quizy from "./Assets/Quizy.png";
import ConnectNow from "./Assets/ConnectNow.png";
import WeatherNow from "./Assets/WeatherNow.png";
import Clothify from "./Assets/Clothify.png";
import Python from "./Assets/PythonCertificate.jpg";
import Snowflake from "./Assets/Snowflake.jpg";
import SQL from "./Assets/SQL-Certificate.jpg";
import TechBoy from "./Assets/TechBoy.png";
import GenAI from "./Assets/GenAI_Cohort.png";
import GenAI1 from "./Assets/IBM-GenAI.jpg";
import GenAI2 from "./Assets/IBM-GenAI(1).jpg";

export const Bio = {
  name: "Suraj Yadav",
  roles: ["Software Engineer", "AI Engineer"],
  description:
    "Software Engineer with 5 years of professional experience in developing scalable web applications and backend services. Specialized in JavaScript with Typescript, Python, React.js, Next.js, FastAPI, Node.js and Snowflake with hands-on experience in AWS, SQL and emerging technologies such as Generative AI. Driven by continuous learning and collaboration to foster innovation.",
  github: "https://github.com/sarry7045",
  resume:
    "https://drive.google.com/file/d/1-CCUvdQ6d1PcP9Vs7O4XGeBn60YFyOPB/view?usp=sharing",
  linkedin: "https://www.linkedin.com/in/suraj-yadav-01752b222/",
  insta: "https://www.instagram.com/suraj.28_11/",
  facebook: "https://www.facebook.com/profile.php?id=100010635822203",
  mail: "mailto: surajyadav112820@gmail.com",
};

export const skills = [
  {
    title: "Frontend",
    skills: [
      {
        name: "JavaScript",
        image:
          "https://upload.wikimedia.org/wikipedia/commons/thumb/6/6a/JavaScript-logo.png/800px-JavaScript-logo.png",
      },
      {
        name: "TypeScript",
        image:
          "https://cdn.jsdelivr.net/gh/devicons/devicon/icons/typescript/typescript-original.svg",
      },
      {
        name: "React Js",
        image:
          "data:image/svg+xml;base64,",
      },

      {
        name: "Next Js",
        image:
          "data:image/png;base64,",
      },
      {
        name: "Redux",
        image:
          "https://d33wubrfki0l68.cloudfront.net/0834d0215db51e91525a25acf97433051f280f2f/c30f5/img/redux.svg",
      },
      // {
      //   name: "React Testing Library",
      //   image:
      //     "https://res.cloudinary.com/dg3gyk0gu/image/upload/v1700500148/tags/testing-library.png",
      // },
      // {
      //   name: "HTML",
      //   image: "https://www.w3.org/html/logo/badge/html5-badge-h-solo.png",
      // },
      // {
      //   name: "CSS",
      //   image:
      //     "https://upload.wikimedia.org/wikipedia/commons/thumb/d/d5/CSS3_logo_and_wordmark.svg/1452px-CSS3_logo_and_wordmark.svg.png",
      // },
      // {
      //   name: "Bootstrap",
      //   image:
      //     "https://getbootstrap.com/docs/5.3/assets/brand/bootstrap-logo-shadow.png",
      // },
      // {
      //   name: "Material UI",
      //   image:
      //     "data:image/png;base64,",
      // },
      {
        name: "Tailwind CSS",
        image:
          "https://w7.pngwing.com/pngs/293/485/png-transparent-tailwind-css-hd-logo.png",
      },
      // {
      //   name: "ANTD",
      //   image:
      //     "https://static-00.iconduck.com/assets.00/ant-design-icon-2048x2046-dl3neb73.png",
      // },
      {
        name: "ShadCN",
        image:
          "https://mediaresource.sfo2.digitaloceanspaces.com/wp-content/uploads/2024/04/20161105/shadcn-ui-logo-EF735EC0E5-seeklogo.com.png",
      },
      // {
      //   name: "React i18Next",
      //   image: "https://translate.i18next.com/i18next.png",
      // },
      // {
      //   name: "Sass",
      //   image:
      //     "https://cdn.jsdelivr.net/gh/devicons/devicon/icons/sass/sass-original.svg",
      // },
      // {
      //   name: "PWA",
      //   image:
      //     "https://uxwing.com/wp-content/themes/uxwing/download/brands-and-social-media/pwa-icon.png",
      // },
      // {
      //   name: "React Hook Form",
      //   image: "https://avatars.githubusercontent.com/u/53986236?s=280&v=4",
      // },
      // {
      //   name: "React Router Dom",
      //   image: "https://www.svgrepo.com/show/354262/react-router.svg",
      // },
    ],
  },
  {
    title: "Backend",
    skills: [
      {
        name: "Node Js",
        image: "https://nodejs.org/static/images/logo.svg",
      },
      // {
      //   name: "Express Js",
      //   image:
      //     "data:image/png;base64,",
      // },
      {
        name: "Python",
        image:
          "https://cdn3.iconfinder.com/data/icons/logos-and-brands-adobe/512/267_Python-512.png",
      },
      {
        name: "FastAPI",
        image: "https://avatars.githubusercontent.com/u/156354296?s=280&v=4",
      },
      {
        name: "MongoDB",
        image:
          "https://raw.githubusercontent.com/devicons/devicon/master/icons/mongodb/mongodb-original-wordmark.svg",
      },
      {
        name: "SQL",
        image:
          "https://w7.pngwing.com/pngs/170/924/png-transparent-microsoft-sql-server-microsoft-azure-sql-database-microsoft-text-logo-microsoft-azure.png",
      },
      {
        name: "Snowflake",
        image:
          "https://p.kindpng.com/picc/s/191-1912861_snowflake-data-warehouse-icon-hd-png-download.png",
      },
      // {
      //   name: "Firebase",
      //   image: "https://www.vectorlogo.zone/logos/firebase/firebase-icon.svg",
      // },
      {
        name: "NGINX",
        image: "https://www.svgrepo.com/show/373924/nginx.svg",
      },
      {
        name: "Web Socket",
        image:
          "https://iotbytes.wordpress.com/wp-content/uploads/2017/01/websocket-logo.png",
      },
      {
        name: "Kafka",
        image:
          "https://openwhisk.apache.org/images/icons/icon-kafka-white-trans.png",
      },
      // {
      //   name: "GraphQL",
      //   image: "https://p7.hiclipart.com/preview/292/912/891/graphql-query-language-representational-state-transfer-application-programming-interface-github.jpg",
      // },
      // {
      //   name: "Gen AI",
      //   image: "https://img.freepik.com/premium-vector/black-white-ai-app-icon_1015334-244.jpg",
      // },
    ],
  },
  {
    title: "GenAI",
    skills: [
      {
        name: "LLM's",
        image: "https://twimlai.com/wp-content/uploads/llms_800x800.png",
      },
      {
        name: "LangChain",
        image:
          "https://registry.npmmirror.com/@lobehub/icons-static-png/latest/files/dark/langgraph-color.png",
      },
      {
        name: "LangGraph",
        image:
          "https://registry.npmmirror.com/@lobehub/icons-static-png/latest/files/dark/langgraph-color.png",
      },
      {
        name: "LangSmith",
        image:
          "https://registry.npmmirror.com/@lobehub/icons-static-png/latest/files/dark/langsmith-color.png",
      },

      {
        name: "LlamaIndex",
        image:
          "https://registry.npmmirror.com/@lobehub/icons-static-png/latest/files/dark/llamaindex-color.png",
      },

      {
        name: "MCP Server's",
        image:
          "https://cdn.prod.website-files.com/647618f73e4f16d380e8eed9/67e414f6be9648cb1ca8d8e4_mcp.png",
      },
      // {
      //   name: "A2A",
      //   image:
      //     "https://companieslogo.com/img/orig/A2A.MI-ea3e20d3.png?t=1730097905",
      // },
      {
        name: "Qdrant",
        image: "https://avatars.githubusercontent.com/u/73504361?v=4",
      },
      {
        name: "Neo4J",
        image:
          "https://images.seeklogo.com/logo-png/27/2/neo4j-logo-png_seeklogo-273740.png",
      },
      // {
      //   name: "Mem0",
      //   image:
      //     "https://media.licdn.com/dms/image/v2/D560BAQEKCL-NcwJ59g/company-logo_200_200/company-logo_200_200/0/1720819731793/mem0_logo?e=2147483647&v=beta&t=Fseh27rIEmtK5XbU3yOlbxOO-ufWJGqqlH-WDhMVLjU",
      // },
    ],
  },
  {
    title: "DevOps",
    skills: [
      {
        name: "Docker",
        image:
          "https://raw.githubusercontent.com/devicons/devicon/master/icons/docker/docker-original-wordmark.svg",
      },
      {
        name: "AWS",
        image:
          "https://logos-world.net/wp-content/uploads/2021/08/Amazon-Web-Services-AWS-Emblem.png",
      },
      {
        name: "Github Actions",
        image:
          "https://avatars.githubusercontent.com/u/44036562?s=280&v=4",
      },
    ],
  },
  {
    title: "Android",
    skills: [
      {
        name: "Android Studio",
        image:
          "https://developer.android.com/static/studio/images/new-studio-logo-1_1920.png",
      },
      {
        name: "React Native",
        image:
          "https://cdn.jsdelivr.net/gh/devicons/devicon/icons/react/react-original.svg",
      },
    ],
  },
  // {
  //   title: "Others Tools",
  //   skills: [
  //     {
  //       name: "Git",
  //       image:
  //         "https://e7.pngegg.com/pngimages/713/558/png-clipart-computer-icons-pro-git-github-logo-text-logo-thumbnail.png",
  //     },
  //     {
  //       name: "BitBucket",
  //       image:
  //         "https://w7.pngwing.com/pngs/404/31/png-transparent-bitbucket-icon-hd-logo.png",
  //     },
  //     {
  //       name: "Stripe",
  //       image:
  //         "https://a.slack-edge.com/2a8628a/img/plugins/stripe/service_512.png",
  //     },
  //     {
  //       name: "StoryBook",
  //       image: "https://www.svgrepo.com/show/354397/storybook-icon.svg",
  //     },
  //     {
  //       name: "Sentry",
  //       image:
  //         "https://e7.pngegg.com/pngimages/282/295/png-clipart-red-illustration-sentry-logo-icons-logos-emojis-tech-companies-thumbnail.png",
  //     },
  //     {
  //       name: "PostHog",
  //       image: "https://www.moengage.com/wp-content/uploads/PostHog-icon.png",
  //     },
  //     {
  //       name: "Motion",
  //       image: "https://cdn.worldvectorlogo.com/logos/framer-motion.svg",
  //     },
  //     {
  //       name: "Vercel",
  //       image: "https://www.svgrepo.com/show/354513/vercel-icon.svg",
  //     },
  //     {
  //       name: "Postman",
  //       image:
  //         "https://static-00.iconduck.com/assets.00/postman-icon-497x512-beb7sy75.png",
  //     },
  //     {
  //       name: "Figma",
  //       image:
  //         "https://s3-alpha.figma.com/hub/file/1481185752/fa4cd070-6a79-4e1b-b079-8b9b76408595-cover.png",
  //     },
  //   ],
  // },
];

export const experiences = [
  {
    id: 0,
    img: "https://mms.businesswire.com/media/20221005005781/en/591798/23/saama_logo_blue_blk_cmyk_highres.jpg",
    role: "Senior Software Engineer",
    company: "Saama Technologies",
    date: "July 2023 - Present",
    desc: `● Designed and implemented multiple POCs on Retrieval-Augmented Generation (RAG) leveraging LangChain,
vector databases, and LLMs to enhance trial data insights and knowledge retrieval.
● Developed and deployed scalable backend APIs using FastAPI, enabling efficient data processing and seamless
integration with ML/AI models.
● Built and optimized frontend components with React.js and ensured smooth API integration with Node.js,
improving user experience and system reliability.
● Collaborated cross-functionally with stakeholders to deliver secure, compliant, and high-performance solutions for
pharma study specifications and trial data analysis.`,
    skills: [
      "JavaScript",
      "TypeScript",
      "ReactJS",
      "Python",
      "Redux",
      "NodeJs",
      "Gen AI",
      "AWS",
      "Docker",
      "Material UI",
      "Tailwind CSS",
      "Motion",
      "Sass",
    ],
  },
  {
    id: 1,
    img: "https://s3.ap-south-1.amazonaws.com/assets.ynos.in/startup-logos/YNOS333312.jpg",
    role: "Frontend Engineer",
    company: "WealthDesk",
    date: "Aug 2022 - Jul 2023",
    desc: "• Develop and maintain pages to match business Requirements. Worked on EWG (Embedded WealthDesk Gateway) Broker Auth Integration, Broker Order Placement, Customer Queries. Reduced API calls by 20%. Dockerized for monitoring. Also Worked on Page Optimization. ",
    skills: [
      "JavaScript",
      "TypeScript",
      "ReactJS",
      "Redux",
      "Sass",
      "ShadCN",
      "StoryBook",
      "Sentry",
      "Docker",
    ],
  },
  {
    id: 2,
    img: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTEU0UFoWbeo0CT0YHHhJxt9O5ogRsQbAEAxA&usqp=CAU",
    role: "ReactJs Developer",
    company: "Paymentz",
    date: "Sep 2021 - Aug 2022",
    desc: "• Worked on M-Transact - Online Payment App, with MVVM Architecture using JavaScript, NodeJs, Redux, PostHog, React-i18Next etc. Integrated API’s using Redux and Converted Figma design to JSX code.",
    skills: [
      "JavaScript",
      "ReactJS",
      "Redux",
      "NodeJs",
      "Sass",
      "Figma",
      "PostHog",
      "React-i18Next",
      "Git",
    ],
  },
  {
    id: 3,
    img: "https://media.licdn.com/dms/image/C510BAQGEWm0Vu9Ye7w/company-logo_200_200/0/1630613156150?e=2147483647&v=beta&t=FYD6GM8BBqkC7aH5xvoD0eQt7f5DmtYr_De2n9ywgvY",
    role: "Web Developer",
    company: "IXAR",
    date: "Sep 2020 - Sep 2021",
    desc: "Worked as a Web Developer and build beautiful Responsive Website using HTML, CSS, ReactJs, Material-UI etc. Also Performed SEO for Website, In order to Rank them on Google Search Engine.",
    skills: [
      "ReactJS",
      "HTML",
      "CSS",
      "BootStap",
      "Figma",
      "React-Router",
      "GitHub",
    ],
    // doc: "https://firebasestorage.googleapis.com/v0/b/flexi-coding.appspot.com/o/1691181448873.jpeg?alt=media&token=ee85eb8f-7247-43cd-9a1d-ce9f58ea62a6",
  },
];

// export const education = [
//   {
//     id: 0,
//     img: "",
//     school: "",
//     date: "Oct 2021 - Sep 2025",
//     grade: "8.10 CGPA",
//     desc: "I am currently pursuing a Bachelor's degree in Computer Science and Engineering at Mumbai University. I have completed 6 semesters and have a CGPA of 8.10. I have taken courses in Data Structures, Algorithms, Object-Oriented Programming, Database Management Systems, Operating Systems, and Computer Networks, among others.",
//     degree: "Bachelor in Information Technolgy",
//   },
// ];

export const projects = [
  // {
  //   id: 6,
  //   title: "TechBoy",
  //   date: "",
  //   description:
  //     "Introducing 'TechBoy,' Uncover the secrets of coding, tech innovations, and industry breakthroughs. Stay curious, stay ahead—dive into the future of technology with us!.",
  //   image: TechBoy,
  //   tags: [
  //     "TypeScript",
  //     "React Js",
  //     "Redux",
  //     "Framer Motion",
  //     "TailWind",
  //     "NodeJS",
  //   ],
  //   category: "web app",
  //   github: "https://github.com/sarry7045/Tech-Boy",
  //   webapp: "https://dev.techboy.in/",
  // },
  {
    id: 11,
    title: "ShopMart",
    date: "",
    description:
      "Introducing 'ShopMart,' a dynamic and user-friendly shopping web application designed to the online shopping experience. ShopMart seamlessly blends cutting-edge technology with an intuitive interface to offer users a personalized and efficient way to discover, explore, and purchase their favorite products.",
    image: Shopmart,
    tags: [
      "JavaScript",
      "React Js",
      "Context API",
      "Framer Motion",
      "Styled Component",
    ],
    category: "web app",
    github: "https://github.com/sarry7045/E-Commerce-Web",
    webapp: "https://shopp-mart.netlify.app/",
  },
  {
    id: 9,
    title: "Rental",
    date: "",
    description:
      "Introducing 'Rental,' web application that redefines the way individuals search for their homes. Rental is a comprehensive house search platform designed to simplify the property hunting process and empower users with the tools they need to find the perfect place to call home",
    image: Rental,
    tags: [
      "JavaScript",
      "Next Js",
      "Context API",
      "Framer Motion",
      "Material UI",
    ],
    category: "web app",
    github: "https://github.com/sarry7045/Rental",
    webapp: "https://rentall.vercel.app/",
  },
  // {
  //   id: 0,
  //   title: "GymHolic",
  //   date: "",
  //   description:
  //     "Introducing 'GymHolic,' a cutting-edge Website designed to elevate your fitness journey by providing a rich collection of gym exercise images and videos. GymHolic is more than just a repository of workouts; it's a comprehensive platform that inspires and guides users towards achieving their fitness goals with a vast library of high-quality visual content.",
  //   image: GymHolic,
  //   tags: [
  //     "JavaScript",
  //     "React Js",
  //     "Context API",
  //     "Framer Motion",
  //     "HTML, CSS",
  //     "Material UI",
  //   ],
  //   category: "web app",
  //   github: "https://github.com/sarry7045/Gymholic",
  //   webapp: "https://gymholic.netlify.app/",
  // },
  {
    id: 5,
    title: "Clothify",
    date: "",
    description:
      "Introducing 'Clothify,' a dynamic and user-friendly shopping web application designed to the online shopping experience. Clothify seamlessly blends cutting-edge technology with an intuitive interface to offer users a personalized and efficient way to discover, explore, and purchase and pay their favorite products.",
    image: Clothify,
    tags: [
      "JavaScript",
      "React Js",
      "Context API",
      "HTML, CSS",
      "GraphQL",
      "Stripe",
      "FireBase",
    ],
    category: "web app",
    github: "https://github.com/sarry7045/Clothify",
    webapp: "https://clothifyshopping.netlify.app/",
  },
  {
    id: 1,
    title: "LearnHub",
    date: "",
    description:
      "Introducing 'LearnHub,' an user-friendly mobile application designed to bring the online education courses to the fingertips of learners everywhere. LearnHub is a comprehensive platform that connects students with a diverse range of courses, fostering continuous learning and personal development.",
    image: LearnHub,
    tags: [
      "JavaScript",
      "React Native",
      "Context API",
      "Andrioid Studio",
      "HTML, CSS",
      "Axios",
    ],
    category: "android app",
    github: "https://github.com/sarry7045/ABC-Company",
    webapp: "https://mern-client-side.netlify.app/",
  },

  {
    id: 10,
    title: "Expense Mate",
    date: "",
    description:
      "Introducing 'Expense Mate,' a straightforward and efficient expense tracker application designed to empower users with the ability to manage their finances effortlessly. ExpenseMate is a user-friendly solution for tracking expenses, offering a practical and organized approach to financial management.",
    image: ExpenseMate,
    tags: [
      "JavaScript",
      "React Js",
      "Material UI",
      "Context API",
      "React-Icons",
    ],
    category: "web app",
    github: "https://github.com/sarry7045/Expense-Tracker",
    webapp: "https://expense-trracker.netlify.app/",
  },
  {
    id: 2,
    title: "Quizy",
    date: "",
    description:
      "Welcome to 'Quizy,' a captivating website that brings the excitement of quizzes right to your fingertips. Quizy is a user-friendly quiz game website designed to entertain, challenge, and engage users with a variety of interesting questions and topics.",
    image: Quizy,
    tags: [
      "TypeScript",
      "React Js",
      "Context API",
      "Bootstrap",
      "Styled Component",
    ],
    category: "web app",
    github: "https://github.com/sarry7045/Quiz-App",
    webapp: "https://quizzzzzz-app.netlify.app/",
  },
  // {
  //   id: 3,
  //   title: "ConnectNow",
  //   date: "",
  //   description:
  //     "Introducing 'ConnectNow,' a versatile video chat application that facilitates seamless communication and connection between individuals. ConnectNow is a user-friendly and efficient platform designed to provide a basic yet reliable video chat experience.",
  //   image: ConnectNow,
  //   tags: ["JavaScript", "React Js", "Socket.IO", "Express", "Nodemon"],
  //   category: "web app",
  //   github: "https://github.com/sarry7045/Video-Chat-App",
  //   webapp: "https://videeo-chatt-app.netlify.app/",
  // },
  // {
  //   id: 4,
  //   title: "WeatherNow",
  //   date: "",
  //   description:
  //     "Introducing 'WeatherNow,' a simple and reliable weather application designed to provide users with real-time weather updates and forecasts at their fingertips. WeatherNow is a user-friendly solution that makes staying informed about the weather conditions effortless and enjoyable.",
  //   image: WeatherNow,
  //   tags: ["JavaScript", "React Js", "HTML", "CSS", "REST API's"],
  //   category: "web app",
  //   github: "https://github.com/sarry7045/Weather-App",
  //   webapp: "https://react-weatther-app.netlify.app/",
  // },
];

export const achievements = [
  {
    id: 1,
    title: "Appreciation Letter (NUIT Solution)",
    description: "",
    image: Appreciation,
    webapp:
      "https://drive.google.com/file/d/157L_ejvhV85tyc9IqikIh8qlh6k7z3gO/view?usp=drive_link",
  },
  {
    id: 2,
    title: "30 Hours GenAI Cohort",
    description: "",
    image: GenAI,
    webapp:
      "https://drive.google.com/file/d/12EElLj3vfG789YgIlKGpUaWuGQsqilBu/view?usp=sharing",
  },
  {
    id: 4,
    title: "Generative AI Essentials for Software Developers",
    description: "",
    image: GenAI2,
    webapp:
      "https://drive.google.com/file/d/1eGZ60lMfJXCV-vpKUdPwzCTUrdoHQnf9/view?usp=sharing",
  },
  {
    id: 9,
    title: "Hackathon 2024",
    description: "",
    image: Hackathon,
    webapp:
      "https://drive.google.com/file/d/1z0q_5ifPMIiQd3-WCO4oz-wlYEomnMc2/view?usp=drive_link",
  },
  {
    id: 3,
    title: "Python Bootcamp (100 Days)",
    description: "",
    image: Python,
    webapp:
      "https://drive.google.com/file/d/1iNvMYBhDJp5BcGXWM6s7heKydp8WzDVI/view?usp=sharing",
  },
  {
    id: 8,
    title: "Snowflake",
    description: "",
    image: Snowflake,
    webapp:
      "https://drive.google.com/file/d/1b6KRpAjm0oBbS8v8ntthPrsi9YhlSvQH/view?usp=sharing",
  },
  // {
  //   id: 8,
  //   title: "SQL Bootcamp",
  //   description: "",
  //   image: SQL,
  //   webapp:
  //     "https://drive.google.com/file/d/1ULT1MIUCI9Pb7L2IcfNT2d7FIdlK_pH8/view?usp=sharing",
  // },
  // {
  //   id: 4,
  //   title: "Advance ReactJs (Udemy)",
  //   description: "",
  //   image: Clothifyy,
  //   webapp:
  //     "https://drive.google.com/file/d/1tIVobJGNrLt1cUCGmCsWB4VvGJ5-bour/view?usp=drive_link",
  // },
  // {
  //   id: 5,
  //   title: "Generative AI (Saama)",
  //   description: "",
  //   image: GenerativeAI,
  //   webapp:
  //     "https://drive.google.com/file/d/1y3Wi6BC6iG143v63gPdcaw5_GO_Q-Rkt/view?usp=drive_link",
  // },
  // {
  //   id: 6,
  //   title: "Shining Star (Saama)",
  //   description: "",
  //   image: ShiningStar,
  //   webapp:
  //     "https://drive.google.com/file/d/1rlrofKTW0dxcqy8P_JWHRxK7y6LnDD-e/view?usp=drive_link",
  // },
  // {
  //   id: 7,
  //   title: "Saama One Year Appreciation",
  //   description: "",
  //   image: SaamaOneYear,
  //   webapp:
  //     "https://drive.google.com/file/d/1qCoiRvKqGZTvDnY_x3FDekxmm4eUWsH2/view?usp=sharing",
  // },
];
